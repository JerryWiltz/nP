		...
		var doTest = function doTest() {
			...
			for (F = startFreq * frequencyUnits; F < stopFreq * frequencyUnits; F += stepFreq * frequencyUnits) {				
				(function sweep() {;
					...
					var nodalInputs = [];
					for(row = 0; row < lowPassComponents.length; row++) {
						nodalInputs.push([lowPassComponents[row][1], row + 1, row + 2]);
					};
					nodalInputs.push(['out', 1, lowPassComponents.length + 1]);
					lowPassComponents[lowPassComponents.length - 1][2] = nport('nodal', nodalInputs);
